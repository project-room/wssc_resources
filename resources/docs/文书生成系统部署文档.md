...
应用成功部署后, 通过命令行的方式初始化 admin 用户. 执行如下命令:
...
```
当出现 Password 时, 输入 admin 用户密码, 按下 Enter 键确认输入. 这时会再次提示输入密码, 继续输入. 注意密码有一定要求, 不能过于简单. 创建成功后, 控制台会打印: 

```
Superuser created successfully.
```
接下来就可以使用 admin 用户登录系统了.

4.2.7 初始化模板标签, 阶段和使用机构数据.

在 Django 的 admin 页面, 管理员可以自行添加这些数据. 管理员也可用通过 Django `load` 方式来进行. 具体的, 参考 Django 文档. 
...
4. 使用 360 浏览器或 chrome 浏览器访问 `http://<ip>:<port>/` 进行登录, 初始管理员用户名和密码是 `admin:rootroot`, 登录成功, 系统即部署完成.
